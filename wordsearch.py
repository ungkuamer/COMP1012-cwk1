...
def coloured_display(grid: list, positions: list) -> None:
    current_row = []
    count = 0
    # insert code to sort positions according to x-coordinate

    for i in range(0, len(grid)): # refers to y-coordinate
        current_row = [x for x in grid[i]]

        for j in range(0, len(current_row)):
            if (i == coordinate_y) and (j == coordinate_x):
                # print current text with colour
                pass
            else:
                # print text like usual
                pass
...
def is_valid_index(puzzle, line, column):
